...
BRANCH_MP_REGEX = re.compile(
    r'^https?://(?:code\.)?launchpad\.net/'
    r'(?P<branch>~(?P<owner>[-.\w]*)(/[-.\w]+){2,4})'
    '(/\+merge/\d+)?/?$')

RESOURCE_TYPE_LINK_SERIES = "https://api.launchpad.net/devel/#project_series"
...
class LeankitCard(Converter):
    ...
    def save(self):
        ...
        if isinstance(self.lane.board, LeankitTaskBoard):
        ...
        try:
            ...
            print("Error saving card: '%s'" % e.message)
            print("Card data: %s" % data)
            print("Skipping...")
            return
    ...
    def parsed_description(self):
        ...
        if match:
            ...
            try:
                ...
                print "Unable to parse card %s: %s" % (self.id, ex.message)
                annotations = Record()
            return (
                annotations,
...
class LeankitBoard(Converter):
    ...
    def _classifyCards(self):
        ...
        for card in self.cards:
            ...
            if card.external_system_url:
                url = card.external_system_url.encode('ascii', 'ignore')
                self._cards_with_external_links.add(card)
                if BRANCH_MP_REGEX.match(card.external_system_url):
                    self._cards_with_branches.add(card)
                else:
                    print ("WARNING: Card won't sync. Non-branch link ({}) "
                           "for {}.".format(url, card.title))
        print "   - %s feature cards" % len(
            self._feature_cards)
    ...
    def _populateFeatureTagPaths(self):
        """Create a map from feature tag to the group owning the feature."""
        for card in self._feature_cards:
            if card.tags:
                for tag in card.tags.split(","):
                    self.feature_tag_paths[tag] = card.lane.path
